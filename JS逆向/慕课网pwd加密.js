var K = function(a) {
    ...
    function k(b) {
        for (var c = new h, r = b.length, d = 0; 0 < r; r -= 4, ++d) {
    ...
    function l(b, c) {
        if (b.isNeg != c.isNeg) {
            ...
            if (-1 == a) {
                ...
                a = 0 - Number(0 > a);
                r.isNeg = !b.isNeg
            } else r.isNeg = b.isNeg
        }
        return r
    }

    function n(b) {
        for (var c = b.digits.length - 1; 0 < c && 0 == b.digits[c];)--c;
        return c
    }
    ...
    function p(b, c, a, d, m) {
        for (m = Math.min(c + m, b.length); c < m; ++c, ++d)
        a[d] = b[c]
    }
    ...
    function y(b, c) {
        ...
        c %= 16;
        a = 16 - c;
        for (var m = d.digits.length - 1, e = m - 1; 0 < m; --m, --e)
    ...
    function L(b, a) {
        ...
        a %= 16;
        c = 16 - a;
        for (var e = 0, f = e + 1; e < d.digits.length - 1; ++e, ++f)
    ...
    function M(a, c) {
        if (a.isNeg != c.isNeg) return 1 - 2 * Number(a.isNeg);
        for (var b = a.digits.length - 1; 0 <= b; --b)
        if (a.digits[b] != c.digits[b]) return a.isNeg ? 1 - 2 * Number(a.digits[b] > c.digits[b]) : 1 - 2 * Number(a.digits[b] < c.digits[b]);
        return 0
    }
    ...
    function F(a) {
        ...
        g = u(a);
            ...
            for (d = C(c, b - q); - 1 != M(g, d);)++f.digits[b - q],
            g = l(g, d);
            for (; b > q; --b) {
                ...
                for (var A = f.digits[b - q - 1] * (65536 * B + z), x = 4294967296 * d + (65536 * p + t); A > x;)--f.digits[b - q - 1],
                ...
                result.digits[1 + p] = A;
                g = l(g, result);
                g.isNeg && (g = w(g, d), --f.digits[b - q - 1])
            }
            g = L(g, k);
            f.isNeg = a.isNeg != e;
